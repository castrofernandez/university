circo.intermedios.Resultado4 = function(canvas)
{
	this.identificador = "resultado4";
	this.imagenes = [];
	...
	function dibujarFondo(contexto, ancho, alto, graficos)
	{
		...
		contexto.fillText(resultado.lateralidad == 0 ? diestro : zurdo, ancho / 2, papel.y + papel.alto / 3.2);
	...
	function pulsarBotonSi() {
		registrarDato("LATERALITY_SUCCESS", "TRUE");
		registrarDato("LATERALITY_VALUE", resultado.lateralidad == 0 ? "RIGHT" : "LEFT");
		pulsado = true;
	};

	function pulsarBotonNo() {
		registrarDato("LATERALITY_SUCCESS", "FALSE");
		registrarDato("LATERALITY_VALUE", resultado.lateralidad == 0 ? "LEFT" : "RIGHT");
		pulsado = true;
	};
